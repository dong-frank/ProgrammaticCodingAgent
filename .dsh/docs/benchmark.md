...
| 任务 id | 类型 | 描述 | 预期特征 |
| --- | --- | --- | --- |
| single-func-fix | 单函数修复 | 修复 add 实现的符号错误 | 最简任务，两种模式差异小 |
| queue-impl | 数据结构实现 | 按规格实现先进先出队列 | 实现类，需要读规格、处理边界 |
| refactor-legacy | 跨文件批量重构 | 替换旧接口调用并删除旧定义 | 多处一致修改，Code Mode 单程序遍历更有优势 |
| csv-parser | 规格实现 | 按规则实现 CSV 解析 | 复杂规格，需处理引号与转义边界 |
| calculator-scaffold | 脚手架生成 | 从空目录生成计算器模块 | 多文件生成，Code Mode 单次程序可完成全部生成 |

任务按难度递进，覆盖修复、实现、重构、生成四类，便于观察不同任务特征下两种模式的表现差异。
