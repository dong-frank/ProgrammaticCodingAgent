...
export function buildCodeSystemPrompt(workspace: string, toolNames: string[]): string {
    return [
        ...
        "程序内可直接使用全局异步函数：readFile(path) 读取文件，writeFile(path, content) 写入文件，shell(command) 执行命令，glob(pattern, ignore?) 匹配路径（需要排除目录时用第二个参数传入模式列表，例如排除依赖目录可传 [\"**/node_modules/**\", \"**/.git/**\"]）。",
